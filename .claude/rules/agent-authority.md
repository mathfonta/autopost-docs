...
### @security (Sage) — Security Audit Authority

| Operação | Exclusivo? | Outros Agentes |
|----------|-----------|----------------|
| `*audit` — OWASP Top 10 completo | SIM | @qa faz verificação superficial (1/7 checks) |
| `*threat-model` — STRIDE por Epic/Story | SIM | BLOQUEADO para outros agentes |
| `*secrets-check` — scan de secrets expostos | SIM | BLOQUEADO |
| `*rls-review` — auditoria RLS Supabase | SIM | BLOQUEADO |
| Criar stories de remediação diretamente | SIM (aprovado Matheus 2026-04-25) | Outros passam por @po |

**Restrições:** @security é READ-ONLY — nunca modifica código, banco ou infraestrutura.
