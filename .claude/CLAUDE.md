...
---
## 🧠 SEGUNDO CÉREBRO — REGRA 0 — EXECUÇÃO OBRIGATÓRIA

**Esta é a primeira regra. Nenhuma ação, resposta técnica,
implementação ou análise pode ocorrer antes desta leitura.**

---

### Ao iniciar qualquer sessão, execute na ordem:

1. Leia `G:\Meu Drive\OBSidian\AutoPost\📊 Status do Projeto.md`
   → Entenda o que está feito, o que está pendente, qual story está ativa
...
### Se arquivo ou seção não existir

**Arquivo ausente:**
Não tente criar. Avise imediatamente:
> "⚠️ Arquivo não encontrado: [caminho completo].
> Crie no Obsidian e retome a sessão."
> Aguarde confirmação de Matheus antes de continuar.

**Seção ausente dentro de arquivo existente:**
Crie a seção no arquivo com o cabeçalho adequado e registre normalmente.
Exemplo: se `APRENDIZADOS.md` não tiver seção "Meta API", crie:
---

### Após a leitura, confirme obrigatoriamente com:

> "🧠 Cérebro carregado.
> Story ativa: [nome da story]
> Último aprendizado registrado: [resumo de 1 linha]
> Ideias novas desde última sessão: [sim — o quê / não]
>
> 📊 Git Status:
> backend/: [N arquivos modificados | LIMPO] | [N commits não pushed | SINCRONIZADO]
> frontend/: [N arquivos modificados | LIMPO] | [N commits não pushed | SINCRONIZADO]
>
> Pronto para continuar."

Só depois disso responda qualquer pergunta ou execute qualquer tarefa.

---

### Quando registrar no Cérebro

O Cérebro cresce com o projeto. Registre sempre que:

**→ APRENDIZADOS.md**
- Resolveu um bug não óbvio → erro, causa, solução
- Tomou decisão de arquitetura → documente o porquê
- Tentou algo que não funcionou → documente para não repetir
- Descobriu padrão útil no codebase → documente

**→ Status do Projeto.md**
- Concluiu uma story ou subtarefa relevante
...
### Atualização em cascata — OBRIGATÓRIA ao concluir Epics/Stories

Ao marcar qualquer Epic ou Story como **Done/Concluído**, atualizar TAMBÉM:

| Arquivo | O que atualizar |
|---------|----------------|
| `Produto/Roadmap.md` | Marcar item como `[x]`, adicionar data de conclusão |
| `🏠 Home.md` | Atualizar bloco "Status Atual" com novo estado |
| `🌐 Global/projetos/autopost.md` | Atualizar lista de epics e status geral |
| `Produto/Arquitetura.md` | Atualizar se stack ou pipeline mudou |

**Regra:** estes arquivos devem sempre refletir a realidade atual do projeto.
Nunca deixar um Epic Done sem atualizar o Roadmap e o Home.

---

---

### Gatilhos automáticos de releitura

| Situação | O que reler |
|---|---|
| Início de sessão | Status + APRENDIZADOS + Ideas |
| Antes de decisão de arquitetura | APRENDIZADOS → seção "Arquitetura" |
| Antes de mexer na Meta/Instagram API | APRENDIZADOS → seção "Meta API" |
| Antes de qualquer mudança no frontend | APRENDIZADOS → seção "Frontend" |
| Encontrou bug difícil | APRENDIZADOS completo antes de tentar |
| Matheus menciona algo que "já foi resolvido" | APRENDIZADOS completo |

---

### Propósito

O AIOX já cria contexto e memórias para continuidade de tasks.
O Segundo Cérebro tem papel diferente e complementar:

> Acumular bagagem real — erros cometidos, soluções encontradas,
> decisões tomadas — para que cada sessão seja mais inteligente
> que a anterior. É a experiência do projeto, não só o estado.
